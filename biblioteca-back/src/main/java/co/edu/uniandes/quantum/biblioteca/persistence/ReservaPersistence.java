...
public class ReservaPersistence
{
 private static final Logger LOGGER = Logger.getLogger(ReservaPersistence.class.getName());    
    ...
    public ReservaEntity find(Long usuarioid, Long Reservaid) {
     ...
     public List<ReservaEntity> findAll(Long usuarioid) 
     {
        TypedQuery<ReservaEntity> q = em.createQuery("select p from ReservaEntity p where (p.miUsuario.id = :usuarioid) ", ReservaEntity.class);
        q.setParameter("usuarioid", usuarioid);      
        List<ReservaEntity> results = q.getResultList();       
        return results;
    }   
